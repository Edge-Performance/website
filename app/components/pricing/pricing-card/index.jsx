...
export default function PricingCard({ hook, description, annualPrice, monthlyPrice, ctaButton }) {
  return (
    <div className="bg-indigo_900_05 h-full flex flex-col items-start justify-between p-6 sm:px-5 rounded-[32px] w-full gap-5">
      ...
      <div className="flex flex-col gap-2 items-start justify-start mt-2 w-full">
        ...
        <div className="flex flex-row gap-2 items-end justify-start mr-auto mt-[33px] self-stretch w-auto">
          <p className="font-bold sm:text-4xl md:text-[42px] text-[46px] text-left text-white_A700 w-auto">
          ${annualPrice}/
          </p>
          <p className="font-normal not-italic text-left text-lg text-white_A700 w-auto">Annualy</p>
      </div>
      {monthlyPrice && (
        <div className="flex flex-row gap-2 items-end justify-start self-stretch w-auto">
        <p className="font-bold text-blue_400 text-left text-lg w-auto">${monthlyPrice}/</p>
        <p className="font-normal not-italic text-blue_400 text-left text-lg w-auto">Monthly</p>
  </div>
        )}
        </div>
      <Button
