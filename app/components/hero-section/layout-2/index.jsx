...
export default function HeroSection({ heroImage, title, ctaButton }) {
  return (
    ...
    <div className="absolute inset-0 overflow-hidden flex items-end justify-end sm:items-center sm:justify-center">
      <div className="flex flex-col items-center justify-between sm:justify-center h-[58%] sm:h-[10%] gap-10 mx-10 sm:mx-0 mb-9 sm:mb-0">
        <h1 className="font-medium capitalize text-[42px] sm:text-6xl text-white_A700 text-center sm:text-left leading-[48px] sm:leading-[70.00px] sm:tracking-[-3.30px]">
            {title}
        </h1>
        { ctaButton && <Button url={ctaButton.url} className="w-full sm:w-auto bg-amber_A400 text-black" text={ctaButton.text} />}
      </div>
    </div>
</section>
  )
